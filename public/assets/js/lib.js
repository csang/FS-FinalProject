...
var lib = function(elems){
	return new lib.prototype.init(elems);
};

lib.prototype = {
	
	init: function(elems){
		if(elems.nodeType){ // Single Element, Create an array with 1 value
			this.elements = [elems];
		}else{
	...
	each: function(callback){ // Callback is the function declared in main.js
		for(var i=0, max=this.elements.length; i<max; i++){
			callback.call(this.elements[i]); // element.call(CONTEXT)
		};
	},
	...
	animate: function(options){
		...
		this.each(function(){
			...
			for(var prop in options.css){
				...
				startCss[prop] = {};
				startCss[prop].start = parseFloat( lib(elem).getStyle(prop) ); 
